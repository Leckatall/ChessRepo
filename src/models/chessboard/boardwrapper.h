...
public:
    ...
    void undo_move(Models::Move m) {
        return unmakeMove(move(m));
    }
    Models::Move make_move(Models::Move m) {
        // Shouldn't be an override like this should have this be the only method making moves
        makeMove(chess::uci::uciToMove(*this, m.uci.toStdString()));
        return m;
    }

    Models::Move make_move(const QModelIndex &from, const QModelIndex &to) {
